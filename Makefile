...
test-master-branch-image:
	...
	@$(call status_after_run, docker run --rm --volume $${PWD}:/app --workdir /app $($@_IMAGE_NAME) $(COMMAND_WITH_FLAG))

	@printf "Container understands only a flag \"$(COMMAND_FLAG_ONLY)\": "
	@$(call status_after_run, docker run --rm --volume $${PWD}:/app --workdir /app $($@_IMAGE_NAME) $(COMMAND_FLAG_ONLY))

	@printf "Container understands other commands: "
	@$(call status_after_run, docker run --rm --volume $${PWD}:/app --workdir /app $($@_IMAGE_NAME) test -x "$$(command -v ls)")

	@printf "Container understands entrypoint override: "
	@$(call status_after_run, docker run --rm --volume $${PWD}:/app --workdir /app --entrypoint "" $($@_IMAGE_NAME) ls "$$(command -v ls)")

	@printf "Container version contains \"$(DOCKERFILE_VERSION)\": "
	@$(call status_after_run, \
		docker run --rm --volume $${PWD}:/app --workdir /app $($@_IMAGE_NAME) $(COMMAND_FOR_VERSION) \
...
test-docker-compose-image:
	...
	@printf "Container understands command with flag \"$(COMMAND_WITH_FLAG)\": "
	@$(call status_after_run, docker-compose --file $(DOCKER_COMPOSE_FILE_PATH) --project-name ci --no-ansi run --rm --volume $${PWD}:/app --workdir /app sut $(COMMAND_WITH_FLAG))

	@printf "Container understands only a flag \"$(COMMAND_FLAG_ONLY)\": "
	@$(call status_after_run, docker-compose --file $(DOCKER_COMPOSE_FILE_PATH) --project-name ci --no-ansi run --rm --volume $${PWD}:/app --workdir /app sut $(COMMAND_FLAG_ONLY))

	@printf "Container understands other commands: "
	@$(call status_after_run, docker-compose --file $(DOCKER_COMPOSE_FILE_PATH) --project-name ci --no-ansi run --rm --volume $${PWD}:/app --workdir /app sut test -x "$$(command -v ls)")

	@printf "Container understands entrypoint override: "
	@$(call status_after_run, docker-compose --file $(DOCKER_COMPOSE_FILE_PATH) --project-name ci --no-ansi run --rm --volume $${PWD}:/app --workdir /app --entrypoint "" sut ls "$$(command -v ls)")

	@printf "Container version contains \"$(DOCKERFILE_VERSION)\": "
	@$(call status_after_run, \
		docker-compose --file $(DOCKER_COMPOSE_FILE_PATH) --project-name ci --no-ansi run --rm --volume $${PWD}:/app --workdir /app sut $(COMMAND_FOR_VERSION) \
			| head -n 1 | grep --quiet "$(DOCKERFILE_VERSION)" \
	)
